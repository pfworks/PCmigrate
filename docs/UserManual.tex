...
\begin{enumerate}
    ...
    \item Click the \textbf{{\footnotesize$\nabla$}} arrow next to Export for options: ``Export Only'', ``Export + Create Restore Bundle'', ``WSL Only'', or ``Optimize WSL + Export''.
...
\begin{itemize}
    ...
    \item Live log output showing progress (selectable and copyable)
    \item Progress bar
...
Move the external drive to the new machine, then double-click \texttt{Restore.cmd}:
...
\begin{enumerate}
    ...
    \item Double-click \texttt{Restore.cmd} from the export folder (or use the GUI's Restore button).
...
\subsection{Optimizing WSL Before Export}

Use the \texttt{-OptimizeWsl} flag (or the ``Optimize WSL + Export'' option in the GUI) to speed up the export:

\begin{lstlisting}[language=bash]
.\Migrate-Machine.ps1 -OutputPath "E:\PCmigrate" -OptimizeWsl
\end{lstlisting}

This performs two optimizations before exporting:

\begin{enumerate}
    \item \textbf{Converts WSL~1 distros to WSL~2} --- WSL~2 distros export as a VHDX disk copy (fast) rather than a tar archive of every file (slow). This is a one-time, permanent conversion.
    \item \textbf{Compacts VHDX disk images} --- WSL~2 virtual disks grow over time but never shrink automatically. Compacting reclaims unused space, making the export file smaller and the copy faster. Uses \texttt{Optimize-VHD} (Hyper-V) if available, otherwise falls back to \texttt{diskpart}.
\end{enumerate}

\noindent\textbf{Note:} The WSL~1 to WSL~2 conversion is permanent and requires the WSL~2 Linux kernel to be installed. In CLI mode without \texttt{-OptimizeWsl}, the script will prompt interactively if WSL~1 distros are detected.
...
\begin{description}[style=nextline]
    ...
    \item[\texttt{Restore.cmd}] CMD launcher for the restore script. Handles UAC elevation, unblocks zone-marked files, and invokes \texttt{Restore-Machine.ps1}. This is the recommended entry point after extracting a restore bundle.
...
\begin{itemize}
    ...
    \item The \texttt{-Bundle} zip stores WSL exports (\texttt{.tar}/\texttt{.vhdx}) uncompressed since they do not compress meaningfully. All other files are fully compressed.
...
\begin{description}[style=nextline]
    ...
    \item[Copying text from the log window]
    The log output panel supports text selection. Click and drag or use Ctrl+A to select all, then Ctrl+C to copy.

    \item[Restore script does nothing when right-clicked ``Run with PowerShell'']
    Files extracted from a downloaded zip are marked with a Zone Identifier that blocks PowerShell execution. Use \texttt{Restore.cmd} instead---it unblocks the scripts automatically. Alternatively, right-click each \texttt{.ps1} file, open Properties, and check ``Unblock'' before running.

    \item[Convert/Compact WSL reports ``No distros found'' despite having distros]
    This was a bug in versions prior to v0.4.8 where \texttt{wsl -l -v} output (UTF-16LE with embedded null characters) was not being cleaned before parsing. Update to the latest version to resolve this.
\end{description}

%----------------------------------------------------------------------
\section{Continuous Integration \& Releases}

The project includes GitHub Actions workflows for automated builds and testing:

\begin{description}[style=nextline]
    \item[\texttt{.github/workflows/build-installer.yml}] Runs on every push and pull request to \texttt{master}. Compiles the Inno Setup installer, runs the export script, and verifies the output structure.
    ...
    \item[\texttt{.github/workflows/test-wsl.yml}] Runs on push/PR to \texttt{master} and version tags. Tests WSL~1 to WSL~2 conversion and VHDX compaction on a real Windows runner. Script output is uploaded as downloadable artifacts for inspection.
\end{description}
...
\begin{description}[style=nextline]
    ...
    \item[\texttt{.github/workflows/test-wsl.yml}] CI workflow for testing WSL conversion and compaction.
\end{description}
