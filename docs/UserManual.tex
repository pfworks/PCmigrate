...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Windows 11 Migration Tool}
\fancyhead[R]{User Manual}
\fancyfoot[C]{\thepage}

\title{\textbf{Windows 11 Migration Tool}\\[0.5em]\large User Manual}
\author{}
\date{May 2026}
...
\vfill
\begin{center}
\textit{A complete guide to exporting and restoring your Windows 11 environment.}
\end{center}
\newpage
...
The Windows 11 Migration Tool is a PowerShell-based application (with an optional GUI) that helps you move your environment from one Windows 11 machine to another. It automates the tedious parts of PC migration:

\begin{itemize}
    \item Exporting WSL distributions as \texttt{.tar} archives
...
\begin{enumerate}
    ...
    \item Download \texttt{MigrationTool\_Setup.exe} (installer) or \texttt{MigrationTool\_Portable.zip} (no install required).
    \item For the installer: run it---it creates a Start Menu shortcut and an optional desktop icon.
    \item For portable: extract the zip and run \texttt{MigrationTool.cmd}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Compile \texttt{installer.iss} to produce \texttt{MigrationTool\_Setup.exe}.
...
Simply double-click \texttt{MigrationTool.cmd} or run \texttt{MigrationTool-GUI.ps1} directly from an elevated PowerShell prompt. No installation is required.
...
Launch the application from the Start Menu, the desktop shortcut, or by running \texttt{MigrationTool.cmd}.

\begin{enumerate}
    \item Click \textbf{Browse} to select your external drive or output folder.
    \item Click \textbf{Export} to begin the migration export on the source machine.
...
# Or use the default (Desktop\MigrationExport)
.\Migrate-Machine.ps1
\end{lstlisting}
...
\begin{description}[style=nextline]
    ...
    \item[\texttt{WSL/*.tar}] Each WSL distribution exported as a tar archive.
...
\begin{enumerate}
    ...
    \item \textbf{WSL import} --- Runs \texttt{wsl --import} for each \texttt{.tar} archive in the \texttt{WSL/} folder, restoring your Linux distributions.
\end{enumerate}
...
After running the restore script, complete these manual steps:

\begin{enumerate}
    \item \textbf{Set WSL default users:} For each restored distro, run:\\
...
\begin{itemize}
    \item \texttt{MigrationTool\_Setup.exe} --- full Windows installer
    \item \texttt{MigrationTool\_Portable.zip} --- portable scripts (no installation required)
\end{itemize}
...
\begin{description}[style=nextline]
    ...
    \item[\texttt{MigrationTool-GUI.ps1}] WPF-based graphical interface.
    \item[\texttt{MigrationTool.cmd}] CMD launcher that starts the GUI without a console flash.
